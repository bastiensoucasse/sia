...
void Mesh::sample(const Point2f &sample, Point3f &p, Normal3f &n, float &pdf) const
{
    ...
    const Vertex &v1 = vertexOfFace(faceId, 1);
    const Vertex &v2 = vertexOfFace(faceId, 2);
    p = m_transformation * Point3f(b[0] * v0.position + b[1] * v1.position + (1 - b[0] - b[1]) * v2.position);
    n = m_transformation * Normal3f(b[0] * v0.normal + b[1] * v1.normal + (1. - b[0] - b[1]) * v2.normal);
    n.normalize();
    pdf = 1.f / area();
}
