...
inline const Eigen::Vector3f BVH::positionInFace(int face_id,
                                                 int vertex_id) const {
  return static_cast<Vector3f>(m_pMesh->position(
      pmp::Vertex(m_pMesh->faceIndices()(vertex_id, face_id))));
}
...
void BVH::build(const Mesh *pMesh, int targetCellSize, int maxDepth) {
  ...
  if (m_pMesh->n_faces() <= targetCellSize) {
    ...
    m_nodes.reserve(std::min<int>(
        2 << maxDepth, std::log(m_pMesh->n_faces() / targetCellSize)));
    // compute centroids and initialize the face list
    m_centroids.resize(m_pMesh->n_faces());
    m_faces.resize(m_pMesh->n_faces());
    for (int i = 0; i < m_pMesh->n_faces(); ++i) {
      m_centroids[i] =
          (positionInFace(i, 0) + positionInFace(i, 1) + positionInFace(i, 2)) /
          3.f;
      m_faces[i] = i;
    }
