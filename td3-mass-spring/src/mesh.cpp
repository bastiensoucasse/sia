...
void Mesh::instanciateParticleSystem(ParticleSystem &psys) {
  double mass = 1;
  double pmass = mass / n_vertices();
  double ks = 200, kd = 0.1;

  auto vpositions = get_vertex_property<Point>("v:point");

  psys.particles.resize(n_vertices());
  int index = 0;
  for (auto v : vertices()) {
    psys.particles[v.idx()] = new Particle(
        index, pmass, static_cast<Vector3f>(vpositions[v]).cast<double>(),
        Vector3d::Zero());
    index++;
  }
  ...
  for (auto e : edges()) {
    auto v0 = vertex(e, 0);
    auto v1 = vertex(e, 1);
    double d = norm(vpositions[v0] - vpositions[v1]);
    psys.forces.push_back(new SpringForce(
        psys.particles[v0.idx()], psys.particles[v1.idx()], ks, kd, d));
  }
}

void Mesh::updatePos(const ParticleSystem &psys) {
  auto vpositions = get_vertex_property<Point>("v:point");
  for (auto v : vertices()) {
    vpositions[v] = psys.particles[v.idx()]->x.cast<float>();
  }
  updateNormals();
  updateVBO();
}