...
public:
    ...
    Eigen::Map<Eigen::VectorXf> shifts()
    {
        auto &shifts = get_vertex_property<float>("v:shift").vector();
        return Eigen::Map<Eigen::VectorXf>(shifts.data(), shifts.size());
    }
    ...
    /// after editing any vertex attributes: positions, normals, texcoords, masks, shifts
    /// etc.)
    void updateVBO();
